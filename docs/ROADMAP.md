...
SP-04F9's deterministic succession-death resolution is implemented and locally verified. One exact reserved-system death workflow freezes explicit precedence over designation, typed descendants/adoption, principal spouse, typed collateral, claims, and support; selects, disputes, or records no successor; atomically composes existing death cleanup with personal-wealth and opaque-estate inheritance; records minimal distinct regency evidence; and transfers or explicitly ends/continues player control. Save schema 28 advances succession snapshot/system to v4 with one authenticated exact-F8 schema-27 migration. The zero-warning Release build, 307 focused succession tests, 1,217 Simulation.Core tests, repository validation/test gates, formatter, diff/LFS checks, Ponytail review, and independent verification pass locally; the representative 1,000-character/100-resolution fixture is raw non-threshold evidence and does not establish the full-SP-04 three-second budget. Commit identity and exact-SHA hosted macOS arm64/Windows x64 evidence remain pending before F9 is accepted.
...
29. In progress — SP-04F9 deterministic succession resolution, inheritance, minimal regency hooks, and player continuity pass the complete local repository closeout with save schema 28; exact-SHA hosted evidence remains required before acceptance.

M0, M1, and SP-03 are complete. M2 remains Active and is not complete. SP-04 is Active: SP-04A through SP-04F8 have passing exact-SHA hosted macOS arm64/Windows x64 evidence at their accepted revisions, while F9 has passed local closeout and awaits exact-SHA hosted evidence. SP-05 remains blocked. SP-04G is next after F9 acceptance and is not started.
