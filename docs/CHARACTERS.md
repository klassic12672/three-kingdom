...
Independent architecture and verification review found and remediated two integration gaps: later-day resource events initially retained the turn-start calendar, and stale recipient overflow lacked a campaign-path regression. The corrected tree advances the resource calendar and covers both insufficient-funds and overflow cancellation through registered campaign commands; final re-review found no remaining correctness or architectural issue. Local repository gates passed with 280 Simulation.Core, 71 Game.Content, 6 Game.Application, and 18 repository tests, a zero-warning Release build, unchanged content validation, touched-file formatter verification, `git diff --check`, and `git lfs fsck`. Exact-SHA hosted macOS arm64/Windows x64 evidence remains pending at this local documentation point, so C2 is not yet accepted.
