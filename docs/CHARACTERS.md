...
`WorldSnapshot` contract version remains 1 through additive subsystem properties. Newly captured state registers `simulation.characters@2`, `simulation.relationships@2`, `simulation.character_careers@1`, `simulation.character_resources@1`, and `simulation.character_estate_holdings@1`; `SimulationChecksum` canonicalizes every character descriptor, condition, typed-link, relationship, memory-source, career, wealth-account, ledger-entry, folded-history, estate identity, and estate-owner field.

Current `SaveEnvelope` schema 9 requires complete v2 character and relationship snapshots, complete v1 career, resource, and character-estate-holding snapshots, and all five registered system versions. Omission, null, partial state, unsupported nested versions, and a missing, duplicate, or incompatible system version are rejected. The literal nonempty schema-8 fixture was produced with the exact accepted SP-04C2 source/API at `e2d9590afc409da30aef86226a8d90a0023fbda3` and is authenticated before 8→9 injects empty estate state. Future estate snapshot or system data are forbidden in schema 8; migration preserves the complete pre-C3 snapshot, diagnostics, manifests, and source bytes. The complete 1→2→3→4→5→6→7→8→9 chain remains forward-only; schema-1/2 field compatibility remains explicitly unverified. Legacy standalone snapshots may omit a later subsystem only when the complete omitted state is valid and empty; restored captures write current versions.
