...
The current character packages provide versioned character, family, household, identity, relationship, memory, retinue, patronage, recommendation, employment-history, and personal-wealth state; deterministic authoritative queries; v1/v2 authored-content loading; registered career/social/resource mutation; and save persistence. Estate holdings, household/family/faction/court treasuries, debt, currency and commodities, income and spending, economy and land integration, marriage, romance, birth/death resolution, inheritance, succession, battle integration, historical rosters, shipped character content, and UI remain deferred.
...
`WorldSnapshot` contract version remains 1 through additive subsystem properties. Newly captured state registers `simulation.characters@2`, `simulation.relationships@2`, `simulation.character_careers@1`, and `simulation.character_resources@1`; `SimulationChecksum` canonicalizes every character descriptor, condition, typed-link, relationship, memory-source, career, wealth-account, ledger-entry, and folded-history field.

Current `SaveEnvelope` schema 8 requires complete v2 character and relationship snapshots, complete v1 career and resource snapshots, and all four registered system versions. Omission, null, partial state, unsupported nested versions, and a missing system version are rejected before DTO use. The literal nonempty schema-7 fixture was produced with the accepted SP-04C1 binary at `d5d2705d3516c67a06e127dcfa867a854b37a21f` and is authenticated before 7→8 injects empty resource state. Future resource commands, events, snapshot data, or system registration are forbidden in schema 7; migration preserves prior state and source bytes. The complete 1→2→3→4→5→6→7→8 chain remains forward-only; schema-1/2 field compatibility remains explicitly unverified. Legacy standalone snapshots may omit a later subsystem only when the complete omitted state is valid and empty; restored captures write current versions.
